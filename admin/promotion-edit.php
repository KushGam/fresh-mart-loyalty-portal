...
if (!isset($_GET['id'])) {
    $_SESSION['message'] = "No promotion specified";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle image upload
    $banner_image = $promotion['banner_image']; // Keep existing image by default
    if (isset($_FILES['banner_image']) && $_FILES['banner_image']['size'] > 0) {
        ...
        if (move_uploaded_file($_FILES["banner_image"]["tmp_name"], $target_file)) {
            ...
            if ($banner_image && file_exists("../" . $banner_image) && $banner_image != "Images Assets/default_promotion.png") {
                unlink("../" . $banner_image);
            }
    ...
    if ($stmt->execute()) {
        ...
        $error = "Error updating promotion: " . $con->error;
    }
}
?>
				...
				<div class="user-management">
					...
					<form method="POST" enctype="multipart/form-data" style="max-width: 600px; margin: 0 auto;">
						<div style="margin-bottom: 1rem;">
							...
							<input type="text" name="title" value="<?php echo htmlspecialchars($promotion['title']); ?>" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">Description</label>
							<textarea name="description" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px; height: 100px;"><?php echo htmlspecialchars($promotion['description']); ?></textarea>
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">Start Date</label>
							<input type="date" name="start_date" value="<?php echo $promotion['start_date']; ?>" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">End Date</label>
							<input type="date" name="end_date" value="<?php echo $promotion['end_date']; ?>" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
						</div>
						...
						<div style="margin-bottom: 1rem;">
							...
							<?php if ($promotion['banner_image']): ?>
								<div style="margin-bottom: 1rem;">
									<p>Current image:</p>
									<img src="../<?php echo htmlspecialchars($promotion['banner_image']); ?>" alt="Current banner" style="max-width: 200px; margin-top: 0.5rem;">
								</div>
							<?php endif; ?>
						...
						<div style="margin-bottom: 1rem;">
							...
							<select name="discount_type" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
								...
								<option value="fixed" <?php echo $promotion['discount_type'] === 'fixed' ? 'selected' : ''; ?>>Fixed Amount</option>
							</select>
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">Discount Amount</label>
							<input type="number" name="discount_amount" step="0.01" value="<?php echo $promotion['discount_amount']; ?>" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">Minimum Purchase Amount</label>
							<input type="number" name="minimum_purchase" step="0.01" value="<?php echo $promotion['minimum_purchase']; ?>" required style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: block; margin-bottom: 0.5rem;">Usage Limit</label>
							<input type="number" name="usage_limit" min="0" value="<?php echo $promotion['usage_limit'] ?? ''; ?>" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
							<small style="color: #666;">Leave empty for unlimited usage</small>
						</div>
						
						<div style="margin-bottom: 1rem;">
							<label style="display: flex; align-items: center; gap: 0.5rem;">
								<input type="checkbox" name="is_active" <?php echo $promotion['is_active'] ? 'checked' : ''; ?>>
								Active
							</label>
						</div>
						
						<button type="submit" class="btn btn-edit" style="width: 100%;">Update Promotion</button>
					</form>
				</div>
			</div>
		</div>

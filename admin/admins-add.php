<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		try {
			// Insert into users table with temporary password and role_as = 1 (admin)
			$query = "INSERT INTO users (user_id, user_name, password_hash, email, role_as, must_change_password) 
					 VALUES ('$user_id', '$username', '" . password_hash($temporary_password, PASSWORD_DEFAULT) . "', '$email', 1, 1)";
			mysqli_query($con, $query);
			
			// Get the last inserted id
			$last_id = mysqli_insert_id($con);
			
			// Insert into customer_details table (optional for admin, but keeping for profile completeness)
			...
			mysqli_commit($con);
				...
				$_SESSION['message'] = "Admin added successfully and welcome email sent with temporary password";
			...
			<div class="dashboard_content">
				<div class="user-form">
					...
					<div class="info-message">
						<p>A secure temporary password will be automatically generated and sent to the admin's email address. 
						The admin will be required to change their password upon first login.</p>
					</div>
